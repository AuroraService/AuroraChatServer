...
public class ChatEndpoint {
    ...
    public void message(String message, Session session) throws IOException, EncodeException {
        try {
            ...
            wr.write(message + "\n");
            wr.write(session.getId() + "\n");
            for (Session peer : session.getOpenSessions()) {
                wr.write("Peer:" + peer.getId() + "\n");
                peer.getBasicRemote().sendText("Hello" + message);
            }
            wr.close();
        } catch (Exception ex){
            File log = new File("e:\\error");
            FileWriter wr = new FileWriter(log, true);
            wr.write(ex.getMessage());
            wr.close();
        }
    }
}
